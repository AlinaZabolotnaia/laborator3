...
## Контрольные вопросы
	1.	Команда docker run -ti -p 8000:80 ubuntu bash запускает новый контейнер на основе образа ubuntu, открывает в нём терминал (bash) в интерактивном режиме (-ti) и пробрасывает порт 80 контейнера на порт 8000 хоста (-p 8000:80). Это позволяет обращаться к приложениям, работающим в контейнере на порту 80, через порт 8000 на компьютере пользователя.
	2.	Параметр -p используется для проброса (перенаправления) портов между хостовой машиной и контейнером. Формат записи -p внешний_порт:внутренний_порт позволяет направлять сетевые запросы с порта хоста во внутренний порт контейнера, обеспечивая доступ к приложениям внутри контейнера снаружи.
	3.	Чтобы проверить, работает ли Apache в контейнере, можно открыть браузер и перейти по адресу http://localhost:8000 (если порт 80 контейнера проброшен на порт 8000 хоста). Также можно выполнить в контейнере команду curl localhost или service apache2 status для проверки состояния сервиса Apache.
	4.	Веб-контент в контейнере Apache по умолчанию хранится в директории /var/www/html. Именно в этой папке находятся файлы, которые отображаются в браузере при обращении к серверу Apache.
	5.	Чтобы заменить стандартную страницу сервера своим файлом, нужно скопировать свой HTML-файл в директорию /var/www/html, предварительно удалив или заменив файл index.html. Это можно сделать командой внутри контейнера, например: cp /путь/к/файлу.html /var/www/html/index.html.
	6.	После удаления контейнера все изменения, сделанные внутри него (например, скопированные файлы или настройки), будут потеряны, если они не были сохранены в томах (volumes) или вынесены в постоянное хранилище. Контейнер — это временная среда, и по умолчанию он не сохраняет состояние после удаления.
